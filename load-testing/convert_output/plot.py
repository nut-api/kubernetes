...
def collect_data(data_file,step) :
    time = []
    values = []
    vus = []
    avgs_duration = []
    ...
    with open(data_file,'r') as csvfile:
        ...
        for row in rows:
            ...
            if nVus < last_nVus : # Skip down state
                continue
            if nVus % step != 0 : # Ramp state
                if steady_state : # just end of steady state
                    ...
                    continue
            else:   # Steady state & collect data
                steady_state = True
...
def plot(x1,y1,y1_2,avg,sd,step,p90,p95,duration) :
    ...
    x2_labels = np.arange(step,step*len(nx2)+1,step)
    ...
    ax2.set_xlabel("Virtual Users(VUs)",fontsize=10)
...
def main ():
    ...
    times,values,vus,avgs_duration,_,_,_=collect_data(file,load_step)
    _,_,_,_,sd,p90,p95=collect_data(file2,load_step)
